...
def spike_detection(filtered_signal, electrode_stream, fsample, config_or_save_path, min_TH=None, dat_points_pre_min=20,
    ...
    config_or_save_path: Either a Config_Preprocessing object or a string path to save spike detection results.
    min_TH (float): The minimum threshold for spike detection. If None, uses config.MIN_TH or default -5.
    ...
    """
    # Handle both config object and direct path string for backward compatibility
    if hasattr(config_or_save_path, 'DATA_SAVE_PATH'):
        # It's a config object
        config = config_or_save_path
        save_path = config.DATA_PATH.rpartition('/')[0]  # directory of the data file
        if min_TH is None:
            min_TH = config.MIN_TH
        dat_points_pre_min = getattr(config, 'DAT_POINTS_PRE_MIN', dat_points_pre_min)
        dat_points_post_min = getattr(config, 'DAT_POINTS_POST_MIN', dat_points_post_min)
        max_TH = getattr(config, 'MAX_TH', max_TH)
        chunck_len = getattr(config, 'INTERVAL_LENGTH', chunck_len)
        refrec_period = getattr(config, 'REFREC_PERIOD', refrec_period)
        reject_channels = getattr(config, 'REJECT_CHANNELS', reject_channels)
        file_name = getattr(config, 'FILE_NAME', file_name)
    else:
        # It's a path string (legacy usage)
        save_path = config_or_save_path
        if min_TH is None:
            min_TH = -5  # default threshold

    # Ensure reject_channels is a list
    if reject_channels is None:
        reject_channels = [None]
    ...
    length_of_chunck = chunck_len  # in seconds
    ...
    # finding the number of chunks. (e.g. if recording is 60.5s, no_chunks = 61)
                    ...
                    for e in range(filtered_signal.shape[1]):
                        ...
                        if probable_spike[e] == 1 and not (
                            ...
                            if t_diff > refrec_period * fsample and chunk[index, e] == np.min(
                                ...
                                # whether the spike is 2ms apart and whether it is the true minimum
    ...
    # Handle case where no spikes were detected
    if len(spk) == 0:
        print("Warning: No spikes detected. Creating empty results.")
